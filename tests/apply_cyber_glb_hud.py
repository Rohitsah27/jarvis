...
new_js = """window.HUD_CONFIG = {
      // 1. HARMONIOUS PARTICLE PROPORTIONS (Matching Blueprint Subject ID: PHM-731)
      ...
      // 5. EYE POSITIONS (Exact anatomical alignment with jarvis_cyber_face.glb)
      rightEyeX: 0.116,
      rightEyeY: 0.442,
      leftEyeX: -0.116,
      leftEyeY: 0.442,
    ...
    // =========================================================================
    // 👁️ 1. PRECISION ALMOND CYBER EYES (Subject ID: PHM-731)
    // =========================================================================
    ...
    // =========================================================================
    // 👃 2. PRECISION SCULPTED CYBER NOSE
    // =========================================================================
    ...
    // =========================================================================
    // 👄 3. PRECISION CYBERNETIC MOUTH (Cupid's Bow & Highlight Nodes)
    // =========================================================================
    let mouthRig = null;
    ...
    // =========================================================================
    // 🤨 4. SCULPTED CYBERNETIC EYEBROWS
    // =========================================================================
    function buildCyberEyebrows() {
      ...
      [-1, 1].forEach((sign) => {
        ...
        for (let k = 0; k < stippleCount; k++) {
          ...
          const x = sign * (0.030 + 0.014 * Math.sin(t * Math.PI));
    ...
    // =========================================================================
    // 🌐 5. FLOWING FACIAL STREAMLINES (Subject ID: PHM-731 Blueprint Flow)
    // =========================================================================
    function buildFacialTopographyStreamlines() {
      const group = new THREE.Group();

      // A. Zygomatic Cheek Streamlines (Flowing smoothly from cheekbones to ears)
      const streamCount = 11;
      const ptsPerStream = 50;
      ...
      [-1, 1].forEach((sign) => {
        for (let k = 0; k < streamCount; k++) {
          ...
          const y_start = 0.220 + 0.170 * t_k;
          const x_start = 0.024 + 0.038 * t_k;

          for (let i = 0; i < ptsPerStream; i++) {
            const t = i / (ptsPerStream - 1);
            const px = sign * (x_start + (0.160 + 0.050 * t_k) * Math.pow(t, 0.85));
      ...
      // B. Cranial & Forehead Dome Streamlines
      const fhLines = 13;
      ...
      [-1, 1].forEach((sign) => {
        for (let j = 0; j < jawLines; j++) {
          ...
          for (let i = 0; i < jawPts; i++) {
            const t = i / (jawPts - 1);
            const px = sign * (0.042 + 0.180 * Math.pow(t, 0.90));
      ...
      [-1, 1].forEach((sign) => {
        for (let er = 0; er < earRings; er++) {
          ...
          for (let i = 0; i < earPts; i++) {
            ...
            const px = sign * (0.235 + rEarX * Math.cos(ang));
      ...
      // E. Ambient Floating Stardust Particles
      const dustCount = 260;
    ...
    const LOCAL_MODEL_URL = './jarvis_cyber_face.glb';
    const REMOTE_MODEL_URL = './LeePerrySmith.glb';

    function isPointFiltered(px, py, pz) {
      const cfg = window.HUD_CONFIG || {};
      const rEyeX = cfg.rightEyeX || 0.116;
      const rEyeY = cfg.rightEyeY || 0.442;
      const lEyeX = cfg.leftEyeX || -0.116;
      const lEyeY = cfg.leftEyeY || 0.442;

      // 1. Clean orbital sockets
      const dxL = (px - lEyeX) / 0.070;
      const dyL = (py - lEyeY) / 0.042;
      const inLeftEye = (dxL * dxL + dyL * dyL < 1.0) && (pz > 0.08);

      const dxR = (px - rEyeX) / 0.070;
      const dyR = (py - rEyeY) / 0.042;
    ...
    function initFaceGeometry(rawGeom) {
      ...
      // jarvis_cyber_face.glb is already precisely centered at X=0.0
      const xFaceCenterOffset = 0.0;
      ...
      // 2. Uniform background particle fill
      if (indexAttr) {
      ...
      const rEyeX = cfgEye.rightEyeX || 0.116;
      const rEyeY = cfgEye.rightEyeY || 0.442;
      const lEyeX = cfgEye.leftEyeX || -0.116;
      const lEyeY = cfgEye.leftEyeY || 0.442;

      // 1. Left & Right Cyber Eyes
      const leftEye = buildCyberEye(lEyeX, lEyeY, 0.450, false);
      const rightEye = buildCyberEye(rEyeX, rEyeY, 0.450, true);
      faceRoot.add(leftEye);
      faceRoot.add(rightEye);

      // 2. Sculpted Cyber Nose
...
print("SUCCESS: jarvis_hud.html updated with jarvis_cyber_face.glb integration!")
